function PlotLateralJerk(data, constdata, Idx_x, Idx_y, Idx_yaw, Idx_vel, Idx_err, Idx_suc, Idx_latjerk, Method, GraphSetting, env, Pickup)
    figure(1)
    if env == "oa"
    ...
    for i = 1 : size(data, 1) - 1
        if data(i, Idx_x) == data(i + 1, Idx_x) && data(i, Idx_y) == data(i + 1, Idx_y) && data(i, Idx_yaw) == data(i + 1, Idx_yaw) && data(i, Idx_vel) == data(i + 1, Idx_vel)
            ...
            average_latjerk = data(i + 1, Idx_latjerk);
            x = data(i + 1, Idx_x);
            y = data(i + 1, Idx_y);

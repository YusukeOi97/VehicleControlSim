...
for i = 1 : length(Folderlist(1, :))
    ...
    for j = 2 : DataSize
        if data(j, Idx_u) == data(j - 1, Idx_u) && data(j, Idx_v) == data(j - 1, Idx_v) && data(j, Idx_theta) == data(j - 1, Idx_theta) && data(j, Idx_vel) == data(j - 1, Idx_vel)
            ...
            if data(j, Idx_err_mpc) ~= 0 || data(j, Idx_suc_mpc) ~= 1 || data(j, Idx_cal) > 0.26
            else
...
INPUT_VALIDATION = MATRIX_INPUT(idx, :);
%MATRIX_INPUT(idx(1, 1 : 500), :) = [];
OUTPUT_VALIDATION = MATRIX_OUTPUT(idx, :);
%MATRIX_OUTPUT(idx(1, 1 : 500), :) = [];
...
histogram2(OUTPUT_PREDICTED, OUTPUT_VALIDATION, [30 30], 'DisplayStyle', 'tile', 'ShowEmptyBins', 'On', 'XBinLimits', [0 400], 'YBinLimits', [0 400]);
axis equal
colorbar
ax = gca;
ax.CLim = [0 80];

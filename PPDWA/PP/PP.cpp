...
double PP::PP_steer_control(double x_g, double y_g, double yaw, double vel, double Wheelbase, double& total_time)
{
	Lf = k * vel + Lfc; //calculate look ahead distance
	rear_x = x_g - (Wheelbase / 2) * cos(yaw);
	rear_y = y_g - (Wheelbase / 2) * sin(yaw);
	...
	total_time += (double)(end.QuadPart - start.QuadPart) * 1000 / freq.QuadPart;

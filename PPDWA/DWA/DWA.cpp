...
//void DWA::Calc_inp_g(double init_x, double init_y, double init_yaw, double init_vel, double init_tire_ang, double vel_ref, double ret[], int Step)
//{
//	SetDW(init_vel, vel_ref);
//	
//	QueryPerformanceFrequency(&freq);
...
//		for (double Vel = min_vel; Vel <= max_vel; Vel = Vel + delta_vel)
//		{
//			InitState(init_tire_ang, Vel, Angvel, Step);
//			for (int i = 1; i < DWAPreStep; i++)
//			{
//				x[SmpCount][i] = x[SmpCount][i - 1] + vel[SmpCount] * cos(yaw[SmpCount][i - 1]) * T_delta;
//				y[SmpCount][i] = y[SmpCount][i - 1] + vel[SmpCount] * sin(yaw[SmpCount][i - 1]) * T_delta;
//				yaw[SmpCount][i] = yaw[SmpCount][i - 1] + (vel[SmpCount] * tire_ang[SmpCount][i] / Wheelbase) * T_delta;
//				tire_ang[SmpCount][i] = tire_ang[SmpCount][i - 1] + Angvel * T_delta;
//			}
//			for (int i = 0; i < DWAPreStep; i++)
//			{
//				frenet.Cache_f = frenet.frenetlib.GetFrenet(x[SmpCount][i], y[SmpCount][i], yaw[SmpCount][i], u[SmpCount][i], v[SmpCount][i], theta[SmpCount][i], frenet.Cache_f);
//			}
//			
//			for (int i = 0; i < DWAPreStep; i++)
//			{
